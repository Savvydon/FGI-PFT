...
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.participant_id IS NULL OR BTRIM(NEW.participant_id) = '' THEN
        NEW.participant_id := 'PFT-' || LPAD(nextval('participant_number_seq')::TEXT, 6, '0');
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_generate_participant_id ON participants;
CREATE TRIGGER trg_generate_participant_id
BEFORE INSERT ON participants
FOR EACH ROW EXECUTE FUNCTION generate_participant_id();
